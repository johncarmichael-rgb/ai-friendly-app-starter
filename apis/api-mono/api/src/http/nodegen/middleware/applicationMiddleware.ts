import { corsMiddleware, headersCaching, inferResponseType } from '@/http/nodegen/middleware';
...
import { mountAuthRoutes } from '@/services/AuthRoutes';
 ...
 * Session cookie validation via WorkOS AuthKit
 */
export const authenticationMiddleware = (app: express.Application): void => {
  // Mount hard-coded auth routes (/auth/login, /auth/callback, /auth/logout)
  // These must be mounted BEFORE the session middleware so they are accessible
  // without authentication.
  mountAuthRoutes(app);

  // Validate session cookie and attach sessionData to request
